...
class Verification (commands .Cog ):
    ...
    async def on_member_join (self ,member ):
        ...
        try :
            async with aiosqlite .connect (DATABASE_PATH )as db :
                async with db .cursor ()as cur :
                    await cur .execute (
                    "SELECT user_id, reason, permanent FROM banned_users WHERE guild_id = ? AND user_id = ? AND (permanent = 1 OR unbanned_at IS NULL)",
                    (member .guild .id ,member .id )
                    )
                    result =await cur .fetchone ()

                    if result :
                        user_id ,reason ,permanent =result
                        embed =discord .Embed (
                        title ="⛔ Banned User Detected",
                        description =f"{member .mention} is banned from this server and cannot verify.",
                        color =DISCORD_COLORS ['error'],
                        timestamp =utc_to_ist (discord .utils .utcnow ())
                        )
                        embed .add_field (name ="Reason", value =reason if reason else "No reason provided", inline =False )
                        embed .add_field (name ="Ban Type", value ="Permanent" if permanent else "Temporary", inline =True )
                        ...
                        if log_result and log_result [0 ]:
                            log_channel =member .guild .get_channel (log_result [0 ])
                            if log_channel :
                                await log_channel .send (embed =embed )

                        # Kick the user
                        await member .kick (reason ="User is banned - cannot join server")

        except Exception as e :
            logger .error (f"Error checking ban status on join: {e}")
    ...
    @verification .command (name ="setup",description ="⚙️ Set up the advanced verification system.")
    @blacklist_check ()
    @ignore_check ()
    @commands .has_permissions (administrator =True )
    ...
    @verification .command (name ="ban",description ="🔨 Ban a user from verification (prevents rejoining)")
    @blacklist_check ()
    @ignore_check ()
    @commands .has_permissions (ban_members =True )
    async def verification_ban (self ,ctx ,user :discord .User ,* ,reason :str =None ,permanent :bool =False ):
        """Ban a user from verification system"""
        try :
            async with aiosqlite .connect (DATABASE_PATH )as db :
                async with db .cursor ()as cur :
                    # Check if already banned
                    await cur .execute (
                    "SELECT user_id FROM banned_users WHERE guild_id = ? AND user_id = ? AND (permanent = 1 OR unbanned_at IS NULL)",
                    ...
                    if existing :
                        ...
                        description =f"{user .mention} is already banned from verification.",
                        color =DISCORD_COLORS ['warning']
                        )
                    ...
                    # Add to ban list
                    await cur .execute (
                    "INSERT INTO banned_users (guild_id, user_id, banned_at, banned_by, reason, permanent) VALUES (?, ?, ?, ?, ?, ?)",
                    (ctx .guild .id ,user .id ,utc_to_ist (discord .utils .utcnow ()).isoformat (),ctx .author .id ,reason ,1 if permanent else 0 )
                    )
                    await db .commit ()

            embed =discord .Embed (
            title ="🔨 User Banned from Verification",
            description =f"{user .mention} has been banned from verification.",
            color =DISCORD_COLORS ['success'],
            timestamp =utc_to_ist (discord .utils .utcnow ())
            )
            ...
            # Try to kick if they're in the server
            member =ctx .guild .get_member (user .id )
            if member :
                try :
                    await member .kick (reason =f"Banned from verification by {ctx .author}")
                except discord .Forbidden :
                    pass

        except Exception as e :
            logger .error (f"Error banning user from verification: {e}")
            embed =discord .Embed (
            color =DISCORD_COLORS ['error']
            )
            await ctx .send (embed =embed )

    @verification .command (name ="unban",description ="🔓 Unban a user from verification")
    @blacklist_check ()
    @ignore_check ()
    @commands .has_permissions (ban_members =True )
    async def verification_unban (self ,ctx ,user :discord .User ):
        """Unban a user from verification system"""
        try :
            async with aiosqlite .connect (DATABASE_PATH )as db :
                async with db .cursor ()as cur :
                    await cur .execute (
                    "UPDATE banned_users SET unbanned_at = ? WHERE guild_id = ? AND user_id = ?",
                    (utc_to_ist (discord .utils .utcnow ()).isoformat (),ctx .guild .id ,user .id )
                    )
                    await db .commit ()
                    ...
                    if cur .rowcount ==0 :
                        embed =discord .Embed (
                        title ="⚠️ Not Banned",
                        description =f"{user .mention} is not banned from verification.",
                        color =DISCORD_COLORS ['warning']
                        )
                        return await ctx .send (embed =embed )

            embed =discord .Embed (
            title ="🔓 User Unbanned from Verification",
            description =f"{user .mention} can now verify and rejoin the server.",
            color =DISCORD_COLORS ['success'],
            timestamp =utc_to_ist (discord .utils .utcnow ())
            )
            ...
            logger .error (f"Error unbanning user from verification: {e}")
            embed =discord .Embed (
            color =DISCORD_COLORS ['error']
            )
            await ctx .send (embed =embed )

    @verification .command (name ="banlist",description ="📋 List all banned users")
    @blacklist_check ()
    @ignore_check ()
    @commands .has_permissions (ban_members =True )
    async def verification_banlist (self ,ctx ):
        """List all banned users"""
        try :
            async with aiosqlite .connect (DATABASE_PATH )as db :
                async with db .cursor ()as cur :
                    await cur .execute (
                    "SELECT user_id, reason, permanent, banned_at FROM banned_users WHERE guild_id = ? AND (permanent = 1 OR unbanned_at IS NULL)",
                    (ctx .guild .id ,)
                    )
            ...
            if not bans :
                ...
                description ="There are no users banned from verification.",
                color =DISCORD_COLORS ['success']
                )
                return await ctx .send (embed =embed )
            ...
            ban_text =""
            for user_id ,reason ,permanent ,banned_at in bans :
                user =self .bot .get_user (user_id )
                user_name =user .name if user else f"Unknown ({user_id})"
                ban_type ="🔴 Permanent" if permanent else "⏰ Temporary"
                ban_text +=f"**{user_name}** - {ban_type}\n"
                if reason :
                    ban_text +=f"   Reason: {reason[:50]}...\n"

            embed .description =ban_text [:4096]
    ...
    @verification .command (name ="invite",description ="🔗 Create a verification invite link")
    @blacklist_check ()
    @ignore_check ()
    @commands .has_permissions (create_instant_invite =True )
    async def verification_invite (self ,ctx ,max_uses :int =0 ,max_age :int =0 ):
        """Create an invite link for the verification system"""
        try :
            ...
            async with aiosqlite .connect (DATABASE_PATH )as db :
                async with db .cursor ()as cur :
                    ...
                    if not result :
                        ...
                        description ="Verification system is not set up.",
                        color =DISCORD_COLORS ['error']
                        )
                        return await ctx .send (embed =embed )
            ...
            reason =f"Verification invite created by {ctx .author}"
            )

            embed =discord .Embed (
            title ="🔗 Verification Invite Created",
            description =f"Here's your verification invite link:",
            color =DISCORD_COLORS ['success'],
            timestamp =utc_to_ist (discord .utils .utcnow ())
            )
            ...
            embed .add_field (name ="Expires", value =f"{max_age} seconds" if max_age >0 else "Never", inline =True )
            ...
            async with aiosqlite .connect (DATABASE_PATH )as db :
                await db .execute (
                "INSERT OR REPLACE INTO invite_tracking (invite_code, guild_id, created_by, max_uses, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                (invite .code ,ctx .guild .id ,ctx .author .id ,max_uses if max_uses >0 else None ,utc_to_ist (discord .utils .utcnow ()+timedelta (seconds =max_age )).isoformat ()if max_age >0 else None ,utc_to_ist (discord .utils .utcnow ()).isoformat ())
                )
                await db .commit ()

        except Exception as e :
            logger .error (f"Error creating verification invite: {e}")
            embed =discord .Embed (
            color =DISCORD_COLORS ['error']
            )
            await ctx .send (embed =embed )

    @verification .command (name ="invites",description ="📊 View invite statistics")
    @blacklist_check ()
    @ignore_check ()
    @commands .has_permissions (manage_guild =True )
    async def verification_invites (self ,ctx ):
        """View invite statistics"""
        try :
            async with aiosqlite .connect (DATABASE_PATH )as db :
                async with db .cursor ()as cur :
                    await cur .execute (
                    "SELECT invite_code, created_by, uses, max_uses, expires_at, created_at FROM invite_tracking WHERE guild_id = ?",
                    (ctx .guild .id ,)
                    )
            ...
            if not invites :
                ...
                description ="No verification invites have been created yet.",
                color =DISCORD_COLORS ['success']
                )
                return await ctx .send (embed =embed )

            embed =discord .Embed (
            title =f"📊 Invite Statistics ({len(invites)})",
            color =DISCORD_COLORS ['primary'],
            timestamp =utc_to_ist (discord .utils .utcnow ())
            )

            invite_text =""
            for code ,created_by ,uses ,max_uses ,expires ,created in invites :
                creator =self .bot .get_user (created_by )
                creator_name =creator .name if creator else f"Unknown ({created_by})"
                status ="✅ Active" if (max_uses is None or uses < max_uses ) and (expires is None or expires > utc_to_ist (discord .utils .utcnow ()).isoformat ()) else "❌ Expired"
                invite_text +=f"**{code}** - {status}\n"
                invite_text +=f"   Created by: {creator_name}\n"
                invite_text +=f"   Uses: {uses}/{max_uses if max_uses else '∞'}\n\n"

            embed .description =invite_text [:4096]
            embed .set_footer (text =f"Requested by {ctx .author}")

            await ctx .send (embed =embed )

        except Exception as e :
            logger .error (f"Error viewing invite statistics: {e}")
            embed =discord .Embed (
            color =DISCORD_COLORS ['error']
            )
            await ctx .send (embed =embed )
